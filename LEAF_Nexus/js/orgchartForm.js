...
orgchartForm.prototype.initialize = function() {
	...
	this.dialog = new dialogController(this.prefixID+'xhrDialog', this.prefixID+'xhr', this.prefixID+'loadIndicator', this.prefixID+'button_save', this.prefixID+'button_cancelchange');
};
...
orgchartForm.prototype.getForm = function(UID, categoryID, indicatorID) {
	...
	switch(categoryID) {
	    case 1:    // employee
	        $.ajax({
	            url: "ajaxEmployee.php?a=getindicator&empUID=" + UID + "&indicatorID=" + indicatorID,
	    ...
	    case 2:    // position
	        $.ajax({
	            url: "ajaxPosition.php?a=getindicator&pID=" + UID + "&indicatorID=" + indicatorID,
	            success: function(response) {
	            	dialog.setContent(response);
	            	$('input:visible:first, select:visible:first').focus();
	            	$('input:visible:first, select:visible:first').keypress(function(event) {
	    ...
	    case 3:    // group
	        $.ajax({
	            url: "ajaxGroup.php?a=getindicator&groupID=" + UID + "&indicatorID=" + indicatorID,
	            success: function(response) {
	            	dialog.setContent(response);
	            	$('input:visible:first, select:visible:first').focus();
	            	$('input:visible:first, select:visible:first').keypress(function(event) {
...
orgchartForm.prototype.updateFormDisplay = function(UID, categoryID, indicatorID) {
	var t = this;
	switch(categoryID) {
    	case 1:    // employee
    	    $.ajax({
    	        url: "ajaxEmployee.php?a=getFormContent&indicatorID=" + indicatorID + "&empUID=" + UID,
    	        success: function(response) {

    	            $('#xhrIndicator_' + indicatorID + '_' + categoryID + '_' + UID).empty();
    	            $('#xhrIndicator_' + indicatorID + '_' + categoryID + '_' + UID).html(response);
    	            $('#xhrIndicator_' + indicatorID + '_' + categoryID + '_' + UID).fadeOut(250).fadeIn(250);
    	...
    	case 2:    // position
    		$.ajax({
    	        url: "ajaxPosition.php?a=getFormContent&indicatorID=" + indicatorID + "&pID=" + UID,
    	        success: function(response) {
    	...
    	case 3:    // group
    		$.ajax({
    	        url: "ajaxGroup.php?a=getFormContent&indicatorID=" + indicatorID + "&groupID=" + UID,
    	        success: function(response) {
