...
<input dojoType="dijit.form.CheckBox" type="checkbox" name="{$indicator.indicatorID}[{$idx}]" value="{$option}" />
        ...
        {if $indicator.format == 'fileupload' && ($indicator.isMasked == 0 || $indicator.data == '')}
            <fieldset>
                ...
                {if $indicator.data != ''}
                <span style="background-color: #b7c5ff; padding: 4px; line-height: 20px"><img src="../libs/dynicons/?img=mail-attachment.svg&amp;w=16" /> <b>File Attached:</b> <a href="" target="_blank">{$indicator.data}</a></span>
                {/if}
                </span>
            </fieldset>
        {/if}
